    ...
    @php
        $themeAsset = config('backpack.base.theme') == 'light' ? '' : 'dark-';
    @endphp
    <link rel="icon" href="{{ config('backpack.base.browser_tab_logo') }}" type="image/png">
    <link rel="stylesheet" type="text/css" href="{{ asset("packages/backpack/base/css/{$themeAsset}bundle.css").'?v='.config('backpack.base.cachebusting_string') }}">
    ...
    @if (config('backpack.base.mix_styles') && count(config('backpack.base.mix_styles')))
        ...
        @endforeach
    @endif
    <link rel="stylesheet" type="text/css" href="{{ asset("css/{$themeAsset}backend.css").'?v='.config('backpack.base.cachebusting_string') }}">
    @yield('after_styles')
    @stack('after_styles')
